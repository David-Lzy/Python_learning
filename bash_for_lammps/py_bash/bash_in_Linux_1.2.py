...
compare = []
compare_number = [i for i in range(8, 10)]  #从0到9左闭右开
compare_str = 'Compare'

beta = []
#beta_number = [0.1 * i for i in range(0, 11)]
beta_number = [0.8]#[0.32,0.64,0.10,]
beta_str = 'Beta'
...
log_location = disk_path + '/log/' + log_name + 'log.txt'
...
def try_simulation(tried_number,Linux_Bash,in_file_location):
    try:
        ...
        if tried_number >= 2:
            f.writelines('\nSomething unable fixed by changing seeds! Skip this run!\n')
            return
        else:
            if tried_number == 0:
                f.writelines('\n    ------------------------------------------------------------------')
                f.writelines('    Sorry,something went wrong, and the fail run located in:\n')
                f.writelines('    '+in_file_location)
                f.writelines('\n    Here might be something helpful:\n')
                try:
                    f.writelines('    '+str(error1))
                except Exception :
                    pass
                finally:
