...
compare = []
compare_number = [i for i in range(0,9)]#从0到9左闭右开
compare_str = 'Compare'

beta = []
beta_number = [0.1,0.5,1.0]#[0.1*i for i in range(2,3)]
beta_str = 'Beta'
...
for i in All_parameter:
    j = str(i)+'\n'
    f.writelines(j)
f.writelines('ALl Windows path to run list here\n')
f.writelines('*****************************************\n')
for i in win_path:
    j = str(i)+'\n'
    f.writelines(j)
...
for i in range(len(All_path)):
    in_file_location = Data_location + All_path[i] 
    try:
        subprocess.call('mpirun -np 19 \home\dpmd0\lmp_ubuntu< in.2dplasma',shell=True,cwd=in_file_location)
