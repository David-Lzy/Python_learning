...
for i in range(len(All_parameter)):
    ...
    for j in range(parameter_number):    
        for k in range(path_number):            
            ...
            temp2[j][k] = All_path[k]+All_parameter[i][j]+ '/'
    All_path = []
    Linux_path = []       
    All_path = numpy.append(All_path,temp1)
    Linux_path  = numpy.append(All_path,temp1)
...
for i in range(len(All_path)):
    ...
    in_file_location = program_location + '\\Data\\without_analysis\\' + All_path[i]
    try:
        os.makedirs( in_file_location )
    except Exception as e:
        print(e)
    shutil.copy(program_location+'\\in.2dplasma',in_file_location)
...
'''for i in range(len(Linux_path)):
    change_cd = 'cd /' + Linux_path[i]
    try:
        subprocess.call(change_cd,shell=True)
    except Exception as e:
        print(e)
    try:
        subprocess.call('mpirun -np 19 lmp_li64<in.2dplasma',shell=True)
    except Exception as e:
        print(e)'''